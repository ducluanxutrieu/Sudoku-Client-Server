...
namespace Client
{
    public partial class ClientForm9x9 : Form
    {
        ...
        int[,] twoDPlayer = new int[9, 9];

        //Lưu mảng của đối phương để tiện set màu
        int[,] twoDFromOpponent = new int[9, 9];

        //Lưu mảng ban đầu của server
        int[,] twoDFromServer = new int[9, 9];

        //Lưu mảng khi giá trị của cell thay đổi
        int[,] twoDPlayerTemp = new int[9, 9];
        ...
        {
            e.Graphics.DrawLine(new Pen(Color.Gold, 4), 167, 0, 167, 463);
            e.Graphics.DrawLine(new Pen(Color.Gold, 4), 333, 0, 333, 463);
            e.Graphics.DrawLine(new Pen(Color.Gold, 4), 0, 154, 500, 154);
            e.Graphics.DrawLine(new Pen(Color.Gold, 4), 0, 309, 500, 309);
        }
        ...
        void Send(string s)
        {
            try
            {
                ...
                //    player.socket.Send(Serialize(s));
                //else
                //    player.socket2.Send(Serialize(s));
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString());
        ...
        {
            Array.Clear(twoD, 0, 81);
            Array.Clear(twoDPlayer, 0, 81);
            Array.Clear(twoDFromOpponent, 0, 81);
            Array.Clear(twoDFromServer, 0, 81);
        }
        ...
        //Event Edit cell hiện tại lần cuối cùng
        //Không còn dùng nữa và để cho đẹp
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                if ((temp < 1) || temp > 9)
                {
                    MessageBox.Show("Số nhập vào phải từ 1 đến 9!");
                }
                else
                {
                    MessageBox.Show(string.Format("twoD[{0},{1}]", r, c));

                    twoD[r, c] = temp; //Set lại để kiểm tra cho đúng
                    twoDPlayer[r, c] = temp;

                    if (CheckValid(r, c, twoD[r, c]) == false)
                    {
                        MessageBox.Show("Nhầm rồi bạn ơi");
                        string message = "1x" + ChuyenMangSangChuoi();
                        player.socket.Send(Serialize(message));
                    }
                    else
                    {
                        string message = "1x" + ChuyenMangSangChuoi();
            ...
            {
                MessageBox.Show("Nhập số nguyên từ 1 đến 9!");
            }
        }
        ...
        {
            //BatDauTinhGio();
            string s = "0x" + RoomPlayer.passingRoomID + 9 + CheckCombobox();
            Send(s);
        }
