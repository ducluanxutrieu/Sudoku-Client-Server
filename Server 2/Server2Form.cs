...
namespace Server_2
{
    public partial class Server2Form : Form
    {
        ...
        private string NewGame(int level, int maps, int roomID)
        {
            ...
            roomList[roomID].twoD = new int[4, 4];
            roomList[roomID].twoDCheckFull = new int[4, 4];
            roomList[roomID].twoDFrom2Client = new int[4, 4];
            roomList[roomID].twoDCheck = new int[4, 4];
        ...
        void Connect()
        {
            ...
            Thread Listen = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        server.Listen(100);
                        PlayerServer2 player = new PlayerServer2();
        ...
        Socket server2toServer1;

        void ConnectToServer1()
        {
            IP = new IPEndPoint(IPAddress.Parse("127.0.0.1"), 9999);
            server2toServer1 = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.IP);
            server2toServer1.Connect(IP);

            foreach (Room2 roomSyncServer1 in roomList)
            {
                string tinnhanSyncServer1 = "10" + roomSyncServer1.player1.ID + roomSyncServer1.player1.countTime;
                tinnhanSyncServer1 += roomSyncServer1.player2.ID.ToString() + roomSyncServer1.player2.countTime;
                server2toServer1.Send(Serialize(tinnhanSyncServer1));
                Thread.Sleep(1000);
            }

            Thread.Sleep(1000);

            foreach (Room2 item in roomList)
            {
                item.player1.socket.Send(Serialize("11"));
                item.player2.socket.Send(Serialize("11"));
            }
        }
        ...
        void Receive(object obj)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        case "0x": //New Game
                            int roomiD0x = int.Parse(data.Substring(0, 1));
                            ...
                            if (roomList[roomiD0x].player1.ID != 0 && roomList[roomiD0x].player2.ID != 0)
                            {
                                //Quyết định người nào chơi trước
                                string tinnhan1 = "2x1";
                                string tinnhan2 = "2x2";

                                if (roomList[roomiD0x].player1.ID >= roomList[roomiD0x].player2.ID)
                                {
                                    roomList[roomiD0x].player1.socket.Send(Serialize(tinnhan1));
                                    roomList[roomiD0x].player2.socket.Send(Serialize(tinnhan2));

                                }
                                else
                                {
                                    roomList[roomiD0x].player2.socket.Send(Serialize(tinnhan1));
                                    roomList[roomiD0x].player1.socket.Send(Serialize(tinnhan2));
                                }

                                //Gửi game cho 2 người chơi
                                string tinnhan = "0x" + NewGame(level, map, roomiD0x);
                                roomList[roomiD0x].player1.socket.Send(Serialize(tinnhan));
                                roomList[roomiD0x].player2.socket.Send(Serialize(tinnhan));
                            }
                            ...
                            else
                            {
                                ...
                                roomList[roomiD0x].player1.socket.Send(Serialize(tinnhan3));
                            }
                        ...
                        case "1x": //Playing Game
                            ...
                            string tinnhanSync2 = "5x" + message;
                        ...
                        case "8x": //Lấy thông tin người chơi khi vào phòng
                            ...
                            break;
                        case "9x":
                            roomSocketList.Add(socket.socket);

                            break;
                        case "5x": //Dữ liệu đồng bộ từ server 1
                            string codeSync = data.Substring(0, 2);
                            switch (codeSync)
                            {
                                case "0x": //New Game -- Lưu mảng của server 1 lại
                                    string firstGame = data.Substring(2, 1);
                                    int roomID5x0x = int.Parse(data.Substring(3, 1));
                                    int mapSync = int.Parse(data.Substring(4, 1));
                                    string newGameSync = data.Substring(5);
                                    if (firstGame.Equals("1"))
                                    {
                                        roomList[roomID5x0x].twoD = new int[mapSync, mapSync];
                                        roomList[roomID5x0x].twoDCheck = new int[mapSync, mapSync];
                                        roomList[roomID5x0x].twoDCheckFull = new int[mapSync, mapSync];
                                        roomList[roomID5x0x].twoDFrom2Client = new int[mapSync, mapSync];
                                    }
                                    else
                                    {

                                        Array.Clear(roomList[roomID5x0x].twoD, 0, mapSync * mapSync);
                                        Array.Clear(roomList[roomID5x0x].twoDCheck, 0, mapSync * mapSync);
                                        Array.Clear(roomList[roomID5x0x].twoDCheckFull, 0, mapSync * mapSync);
                                        Array.Clear(roomList[roomID5x0x].twoDFrom2Client, 0, mapSync * mapSync);
                                    }
                                    if (mapSync == 4)
                                        ChuyenChuoiSangMang4Sync(newGameSync, roomID5x0x);
                                    else
                                        ChuyenChuoiSangMang9Sync(newGameSync, roomID5x0x);
                                    break;
                                case "1x": //Playing Game
                                    int roomID5x1x = int.Parse(data.Substring(2, 1));
                                    int playerID5x1x = int.Parse(data.Substring(3, 2));
                                    int mapdangchoi1 = int.Parse(data.Substring(5, 1));
                                    string manchoi1 = data.Substring(6);
                                    if (mapdangchoi1 == 4)
                                        ChuyenChuoiSangMang4(manchoi1, roomID5x1x);
                                    else
                                        ChuyenChuoiSangMang9(manchoi1, roomID5x1x);

                                    if (mapdangchoi1 == 4) //Nếu như đang chơi map 4x4
                                    {
                                        for (int r = 0; r < 4; r++)
                                            for (int c = 0; c < 4; c++)
                                            {
                                                if (roomList[roomID5x1x].twoDFrom2Client[r, c] != 0 && roomList[roomID5x1x].twoD[r, c] != roomList[roomID5x1x].twoDFrom2Client[r, c])
                                                {
                                                    roomList[roomID5x1x].twoD[r, c] = roomList[roomID5x1x].twoDFrom2Client[r, c];
                                                    roomList[roomID5x1x].twoDCheckFull[r, c] = roomList[roomID5x1x].twoDFrom2Client[r, c];
                                                }
                                        ...
                                        for (int r = 0; r < 4; r++)
                                            for (int c = 0; c < 4; c++)
                                            {
                                                if (roomList[roomID5x1x].twoD[r, c] != roomList[roomID5x1x].twoDCheck[r, c])
                                                {
                                                    if (CheckValid4(r, c, roomList[roomID5x1x].twoD[r, c], roomID5x1x))
                                                    {
                                                        if (roomList[roomID5x1x].player1.ID == playerID5x1x)
                                                            roomList[roomID5x1x].player1.countTime++;
                                                        else
                                                            roomList[roomID5x1x].player2.countTime++;
                                                        roomList[roomID5x1x].twoDCheck = roomList[roomID5x1x].twoD.Clone() as int[,];
                                                    }
                                                    else
                                                        roomList[roomID5x1x].twoD[r, c] = 0;
                                                }
                                            }
                                    ...
                                    else //Nếu như đang chơi map 9x9
                                    {
                                        ...
                                        for (int r = 0; r < 9; r++)
                                            for (int c = 0; c < 9; c++)
                                            {
                                                if (roomList[roomID5x1x].twoD[r, c] != roomList[roomID5x1x].twoDCheck[r, c])
                                                {
                                                    if (CheckValid9(r, c, roomList[roomID5x1x].twoD[r, c], roomID5x1x))
                                                    {
                                                        if (roomList[roomID5x1x].player1.ID == playerID5x1x)
                                                            roomList[roomID5x1x].player1.countTime++;
                                                        else
                                    ...
                                    break;
                                case "2x": //Nhận ID người chơi
                                    int roomID5x2x = int.Parse(data.Substring(2, 1));
                                    int playerID5x2x = int.Parse(data.Substring(3));

                                    if (roomList[roomID5x2x].player1.ID == 0)
                                        roomList[roomID5x2x].player1.ID = playerID5x2x;
                                    else
                                        roomList[roomID5x2x].player2.ID = playerID5x2x;
                                    break;
                                case "3x":
                                    int roomID5x3x = int.Parse(data.Substring(2, 1));
                                    int playerID5x3x = int.Parse(data.Substring(3));

                                    if (roomList[roomID5x3x].player1.ID == playerID5x3x)
                                    {
                                        socket.ID = roomList[roomID5x3x].player1.ID;
                                        socket.countTime = roomList[roomID5x3x].player1.countTime;

                                        roomList[roomID5x3x].player1 = socket;

                                        roomList[roomID5x3x].player1.socket.Send(Serialize("5x"));
                                    }
                                    else
                                    {
                                        socket.ID = roomList[roomID5x3x].player2.ID;
                                        socket.countTime = roomList[roomID5x3x].player2.countTime;

                                        roomList[roomID5x3x].player2 = socket;

                                        roomList[roomID5x3x].player2.socket.Send(Serialize("5x"));
                                    }

                                    break;
                                case "7x":
                                    Room2 room5x7x = new Room2();
                                    room5x7x.ID = roomID;
                                    roomList.Add(room5x7x);
                                    roomID++;
                                    break;
                            }
                            break;
                        case "6x":
                            int roomID6x = int.Parse(data.Substring(0, 1));
                            int playerID6x = int.Parse(data.Substring(1));

                            for (int i = 0; i < roomList.Count; i++)
                            {
                                if (roomList[i].player1.ID == playerID6x)
                                {
                        ...
                        case "10":
                            ConnectToServer1();
                            //player.socket.Send(Serialize("cc"));

                            //foreach (Room roomSyncServer1 in roomList)
                            //{
                            //    string tinnhanSyncServer1 = "10" + roomSyncServer1.player1.ID + roomSyncServer1.player1.countTime;
                            //    tinnhanSyncServer1 += roomSyncServer1.player2.ID.ToString() + roomSyncServer1.player2.countTime;
                            //    player.socket.Send(Serialize(tinnhanSyncServer1));
                            //    //Thread.Sleep(1000);
                            //}
                            ...
                            //Thread SyncServer1 = new Thread(() =>
                            //{
                            //    foreach (Room roomSyncServer1 in roomList)
                            //    {
                            //        string tinnhanSyncServer1 = "10x" + roomSyncServer1.player1.ID + roomSyncServer1.player1.countTime;
                            //        tinnhanSyncServer1 += roomSyncServer1.player2.ID.ToString() + roomSyncServer1.player2.countTime;
                            //        player.socket.Send(Serialize(tinnhanSyncServer1));
                            //        Thread.Sleep(1000);
                            //    }
                            ...
                            //    foreach (Room item in roomList)
                            //    {
                            //        item.player1.socket.Send(Serialize("11x"));
                            //        item.player2.socket.Send(Serialize("11x"));
                            //    }
                            //});
                    ...
                    lsvMessage.Items.Add(new ListViewItem() { Text = "Connection accepted from " + socket.socket.RemoteEndPoint });
                    AddMessage(message);
                }
            }
            catch
            {
                for (int i = 0; i < roomList.Count; i++)
                {
                    if (roomList[i].player1.ID == socket.ID)
                    {
                        string playerOut = "6x" + i + socket.ID;
                        AddMessage(playerOut);
                    ...
                    else if (roomList[i].player2.ID == socket.ID)
                    {
                        string playerOut = "6x" + i + socket.ID;
                        AddMessage(playerOut);
        ...
        private void receiveAndAddPlayer(String data, Socket socket)
        {
            ...
            PlayerServer2 player = new PlayerServer2(playerID, 0, socket);
        ...
        {
            PlayerServer2 player1 = new PlayerServer2(0, 0, null);
            PlayerServer2 player2 = new PlayerServer2(0, 0, null);
            Room2 room = new Room2(roomID, player1, player2, "4x4");
        ...
        void ChuyenChuoiSangMang4Sync(string s, int roomID)
        {
            int count = 0;
            for (int r = 0; r < 4; r++)
                for (int c = 0; c < 4; c++)
                {
                    roomList[roomID].twoD[r, c] = Convert.ToInt32((char.GetNumericValue(s[count])));
                    roomList[roomID].twoDCheck[r, c] = Convert.ToInt32((char.GetNumericValue(s[count])));
                    roomList[roomID].twoDCheckFull[r, c] = Convert.ToInt32((char.GetNumericValue(s[count])));
                    count++;
                }
        }
        ...
        void ChuyenChuoiSangMang9Sync(string s, int roomID)
        {
            int count = 0;
            for (int r = 0; r < 9; r++)
                for (int c = 0; c < 9; c++)
